...
for pm in letter star; do
  for pf in {1..9}; do
    for cc in {A..Z}; do
      if test $((${Choice[$cc,female_min]}+${Choice[$cc,male_max]})) -gt 0; then
        Male_Count=0
        Female_Count=0
        ...
        for pg in M F; do
          test $pg = M && Person_Count=$Male_Count || Person_Count=$Female_Count
          test $pg = M && Gender_Max=${Choice[$cc,male_max]} || Gender_Max=$((${Choice[$cc,female_min]}+${Choice[$cc,male_max]}))
          if test $(($Person_Count*$Gender_Max)) -eq 0; then
            true
          elif test $Person_Count -le $Gender_Max; then
            for pn in {1..99}; do
              if test "${Person[$pn,$pf]}" = "$Prefer_Value" -a "${Person[$pn,gender]}" = $pg -a "${Arrange[$pn,$cc]}" != Y; then
                Arrange[$pn,$cc]=Y
                let Person[$pn,luck]+=1
                if test $pg = F -a "${Choice[$cc,female_min]}" -gt 0; then
            ...
            while test $Gender_Max -gt 0; do
              for pn in {1..99}; do
                if test "${Person[$pn,$pf]}" = "$Prefer_Value" -a "${Person[$pn,gender]}" = $pg -a "${Arrange[$pn,$cc]}" != Y -a $Gender_Max -gt 0; then
                  if test $RANDOM -lt $((32768*$Gender_Max/$Person_Count/(1+${Person[$pn,luck]}))); then
                    ...
                    let Person_Count-=1 Gender_Max-=1
                  fi
                fi
                test $pn -eq $Person_Max && break
              done
